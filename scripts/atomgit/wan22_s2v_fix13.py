"""Fix13: replace flash attention with standard SDPA for NPU"""
...
log("=== Patch attention.py: standard attention fallback ===")
att_path = "wan/modules/attention.py"
with open(att_path) as f: att = f.read()

if "scaled_dot_product_attention" not in att:
    old = """    else:
    ...
    new = """    else:
        # Standard attention fallback for NPU (no flash attention)
    ...
    if old in att:
        ...
        log("  patched standard attention fallback")
    else:
        log("  pattern not found, trying simpler approach")
        att = att.replace("assert FLASH_ATTN_2_AVAILABLE", "pass  # flash attn not available on NPU")
        with open(att_path, "w") as f: f.write(att)
        log("  patched assert only")
else:
    log("  already patched")
