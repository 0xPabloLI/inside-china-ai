...
for (const url of urls) {
  try {
    ...
    const text = await page.evaluate(() => {
      ...
      document
        .querySelectorAll(
          "script, style, nav, footer, header, aside, iframe, .ad, .ads, .banner, .popup, .modal, .cookie, .newsletter",
        )
    ...
    if (text && text.length > 500) {
      ...
      const startIdx = lines.findIndex(
        (l, i) =>
          i > 2 &&
          l.length > 50 &&
          !/cookie|subscribe|sign up|log in|menu|search|share|follow/i.test(l),
      );
