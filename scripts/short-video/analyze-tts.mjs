...
const scenes = [
  ...
  },
  {
    ...
    silences: [
      ...
      { start: 5.44, end: 6.34, dur: 0.90 },
      { start: 8.02, end: 8.95, dur: 0.93 },
      { start: 10.11, end: 11.01, dur: 0.90 },
...
for (const scene of scenes) {
  ...
  console.log(`  Avg silence: ${(scene.silences.reduce((s, g) => s + g.dur, 0) / scene.silences.length).toFixed(2)}s`);
  console.log(`  Total silence: ${scene.silences.reduce((s, g) => s + g.dur, 0).toFixed(2)}s (${((scene.silences.reduce((s, g) => s + g.dur, 0) / scene.totalDur) * 100).toFixed(0)}% of scene)`);
