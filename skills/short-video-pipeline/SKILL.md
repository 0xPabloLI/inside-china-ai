...
- **TTS**: F5-TTS-MLX (voice cloning, best quality on Apple Silicon) > XTTS v2 (Craig Gutsy, speed=1.15, MPS hybrid) > Kokoro (am_michael, speed=1.1) > edge-tts > say. Batch mode.
...
**Hook Formula Selection (adapted for news)**:
- [ ] Scene 1 Hook uses one of these formulas (by goal):
  - **T1 Cold-Open Result** (goal: completion) — open on the finished outcome, then promise the how. News default.
  - **T3 Specific-Number Reveal** (goal: saves) — lead with one odd, concrete number. Most save-worthy for news.
  - **T4 Open Loop Question** (goal: comments) — ask the exact question the video will answer.
  - **T6 Bold Claim** (goal: comments) — flat, defensible, contrarian claim with zero qualifier.
  - **T7 Listicle Promise** (goal: saves) — numbered payoff up front, one item per beat.
  - **T9 Tutorial Cold Start** (goal: saves) — skip intro, start the how-to on first action.
- [ ] Last scene (CTA) has **loop-close**: ends on a line that recontextualizes the opening for natural rewatch
- [ ] Spoken line and on-screen text in Scene 1 **differ** (≥80% overlap = FAIL, 50-80% = WARN)
...
| **Opening visual** | Stop the thumb with motion/result/interrupt | Show result (T1) or tension (T8) in frame one; no logo/greeting/slow zoom |
...
|-----------|-------|-----------|
| Font size | **60px** | Matches TikTok native ~60px em (≈3.1% of frame height); readable at phone + thumbnail scale |
...
| Chunks | **≤6 words**, pixel-measured: ≤ 720px hard / 612px soft (85%) | Pixel widths (Helvetica-Bold AFM, `lib/subtitles/measure.mjs`) replace char heuristics |
| Max width | **720px** (180px margins L/R → cue right edge x=900) | Clears the action rail; derived from `SUBTITLE_LANE.maxWidth` |
| Background | **None** (transparent, 3px black outline) | Clean look on any scene; outline provides contrast |
| Position | `MarginV=570` (ASS) | Cue bottom edge y=1350 — below content (ends y1150), above the TikTok caption zone (starts ~y1500) |
| Timing | **wav2vec2 forced alignment** (`text-align.py`) | Per-word timestamps from known text aligned to audio |
| Primary color | Dispatch Blue `#4d8bff` | Spoken words (highlighted) |
| Secondary color | White `#F5F5F5` | Unspoken words (waiting to be spoken) |
| Generation | `lib/subtitles/` (JS; `cues.mjs` → `ass.mjs`) | Pixel-width chunking (cues) + proper ASS escaping/styling/`\kf` tags from word timestamps (ass.mjs) |
...
Pipeline stages: TTS → HTML scenes → Playwright record → (BGM if --bgm) → ASS subtitles → FFmpeg assemble.
...
3. **Agent fixes** the issue directly — edit scene-data.mjs, re-run force-align.py, adjust pipeline, etc.
...
- **Primary goal tag**: which Hook formula was used (T1/T3/T4/T6/T7/T9) + goal (completion/saves/comments/shares)
...
📝 Agent-generated (copy these into TikTok):
  ...
  Hook formula: T1 Cold-Open Result (goal: completion)
...
|-----------|----------------|---------------------------|-------|
| **Hook Scripting** | tt-hook-scripter (10 formulas + goal tags) | Step 2: Hook Formula Selection + Hook Anatomy | This SKILL.md |
...
3. **Re-hook for the first second**: Use T1/T3/T4/T6 formulas to write the 3-layer hook (spoken + on-screen + visual).
...
**Decision framework**:
- Topic is breaking (24h, multi-source) -> **Breaking News** (T1 hook, 30-45s)
- Topic is developing (48h+, multi-angle analysis) -> **Deep Analysis** (T4/T6 hook, 60-70s)
- Ranking/comparison data available -> **Data Reveal** (T3/T7 hook, 45-60s)
- Topic is cool but has educational value -> **Explainer** (T9 hook, 60-90s)
...
# F5-TTS-MLX venv (~/.f5-tts-env, Python 3.11): f5_tts_mlx + whisperx (for text-align.py)
# XTTS venv (~/.xtts-env, Python 3.11): TTS + torch 2.5.1
# Kokoro venv (~/.tts-env, Python 3.12): kokoro
```
...
| `lib/tts/registry.mjs` | TTS engine selector. Engine priority: F5-TTS-MLX > XTTS v2 > Kokoro > edge-tts > say. |
| `lib/tts/f5-mlx.mjs` | F5-TTS-MLX engine adapter (default, best quality on Apple Silicon). |
| `lib/tts/xtts.mjs` | XTTS v2 engine adapter (fallback). |
...
2. `lib/tts/registry.mjs` selects engine (F5 > XTTS > Kokoro > edge > say), produces one MP3/WAV per scene + returns exact durations
...
| Priority | Engine | Speaker | Speed | Venv | Notes |
|----------|--------|---------|-------|------|-------|
| 1 | F5-TTS-MLX | ref_audio + ref_text (cloned) | 1.0 | `~/.f5-tts-env` (Python 3.11) | Best quality on Apple Silicon; batch mode; no silenceremove needed (low amplitude) |
| 2 | XTTS v2 | cloned voice or Craig Gutsy | 1.15 | `~/.xtts-env` (Python 3.11) | Batch mode; MPS hybrid (GPT on MPS, HiFi-GAN on CPU) |
| 3 | Kokoro | am_michael | 1.1 | `~/.tts-env` (Python 3.12) | 54 voices; fastest; user-approved quality |
...
- Ref text: `assets/voice-sample-ref-text.txt` (must match ref audio exactly)
- Duration formula: `duration = ref_dur + target_dur` (not setting causes 0.03s audio)
- F5 audio amplitude is low, **skip silenceremove** (-35dB threshold would delete everything)
- Optional atempo: `export TTS_ATEMPO=1.3` (speed up speech)
- M4A not supported by Python audio libs — must convert to WAV first

**XTTS v2** (fallback):
- `export TTS_SPEAKER_WAV=assets/voice-sample.wav` — XTTS clones from denoised WAV sample. MPS hybrid mode (2x faster than CPU).

**Environment variables**:
- `TTS_ENGINE=f5` / `xtts` / `kokoro` / `edge-tts` / `say` — force specific engine
- `TTS_SPEAKER_WAV=none` — use built-in speaker (disable cloning)
- `TTS_SPEAKER_WAV=/path/to/voice.wav` — custom voice sample
- `XTTS_SPEED=1.3` — override XTTS speed (default 1.15)
- `TTS_ATEMPO=1.3` — override F5 atempo (default: none)

**Known XTTS issues**:
- PyTorch 2.13.0 breaks model loading (`weights_only` default changed) → must use 2.5.1
- XTTS stdout prints `["sentence"]` (sentence-split output) → JSON parser must look for `[{"sceneId"` prefix
...
**TTS engine priority**: F5-TTS-MLX (voice cloning, Apple Silicon) > XTTS v2 > Kokoro > edge-tts > macOS say. F5 skips silenceremove (low amplitude); other engines use FFmpeg `silenceremove` (compress pauses >0.25s → 0.08s retained). Read `lib/tts/registry.mjs` for current settings.
...
**XTTS batch mode** (fallback): `xtts_batch_tts.py` loads the 1.8GB model once and processes all scenes (7 min vs 60+ min for per-scene loading). MPS hybrid mode (GPT on MPS, HiFi-GAN on CPU) — requires patching `xtts.py` lines 577 + 320.
