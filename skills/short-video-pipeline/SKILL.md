...
- **TTS**: F5-TTS-MLX (voice cloning, best quality on Apple Silicon) > Qwen3-TTS (backup) > edge-tts > macOS say. Batch mode. Unified venv: `~/.video-tts-env` (Python 3.12). Full config in `docs/video-production-runbook.md` → TTS Engine Configuration.
...
Subtitles are auto-generated by `lib/subtitles/` (cues.mjs → ass.mjs) from wav2vec2 per-word timestamps. Key params: 60px bold, karaoke `\kf` style, MarginV=570, transparent background with outline. All values derive from `SUBTITLE_LANE` in `lib/safe-zones.mjs`. Full spec in `docs/video-production-runbook.md` → Subtitle Best Practices and `docs/brand-system.md` → Subtitle Specification.
...
> Full configuration (max effort parameters, post-processing matrix, prosody) in `docs/video-production-runbook.md` → TTS Engine Configuration.
