...
const EXTRACT_FN = `(function(selector) {
  // 1. 定位正文容器
  var container;
  if (selector) {
  ...
  if (!container) {
    var candidates = [
      ...
      '#js_content', '.rich-text', '[role="main"]', '.markdown-body'
    ];
  ...
  function walk(node, depth) {
    ...
    // 跳过不可见/噪音元素
    if (tag === 'SCRIPT' || tag === 'STYLE' || tag === 'NOSCRIPT' || tag === 'SVG' || tag === 'IFRAME' || tag === 'INS' || tag === 'FORM') return '';
    ...
    if (tag === 'A') {
      ...
      if (href && href !== '#' && !href.startsWith('javascript:') && !href.startsWith('void')) {
        return '[' + text + '](' + href + ')';
      }
      return text;
    }
    ...
    if (tag === 'UL' || tag === 'OL') {
      ...
      for (var j = 0; j < items.length; j++) {
        ...
        var itemText = items[j].textContent.trim().replace(/\\n/g, ' ');
        list += prefix + itemText + '\\n';
      }
      return list + '\\n';
    }
    ...
    if (tag === 'TABLE') {
      ...
      for (var r = 0; r < rows.length; r++) {
        var cells = [];
        rows[r].querySelectorAll('th,td').forEach(function(c) {
          cells.push(c.textContent.trim().replace(/\\n/g, ' '));
        });
        if (cells.length === 0) continue;
        table += '| ' + cells.join(' | ') + ' |\\n';
        if (!headerDone) {
          table += '| ' + cells.map(function() { return '---'; }).join(' | ') + ' |\\n';
          headerDone = true;
        }
      }
      return table + '\\n';
    ...
    // 段落、div、section、figure、figurecaption → 递归遍历子节点
    for (var k = 0; k < node.childNodes.length; k++) {
      result += walk(node.childNodes[k], depth + 1);
    }

    if (tag === 'P' || tag === 'DIV' || tag === 'SECTION' || tag === 'FIGURE' || tag === 'FIGCAPTION') result += '\\n\\n';

    return result;
  }

  // 3. 执行转换并清理
  var md = walk(container, 0);
  // 清理多余空行（3+ 换行压缩为 2 个）
  md = md.replace(/\\n{3,}/g, '\\n\\n').trim();
  return md;
})`;
  ...
  return new Promise((resolve) => {
    ...
    const checkInterval = setInterval(async () => {
      try {
        ...
        const ready =
          state?.ready === "complete" || (acceptInteractive && state?.ready === "interactive");
...
const server = http.createServer(async (req, res) => {
  ...
  try {
      ...
      if (req.method !== "POST") {
        ...
        res.end(
          JSON.stringify({
            ...
            migration: "references/migration-2.5.3.md",
            example:
              "curl -X POST --data-raw 'https://example.com' 'http://localhost:3456/navigate?target=ID'",
          }),
        );
        return;
      }
    ...
    // POST /extract?target=ID — 提取正文为 Markdown
    // body: JSON { "selector": "#js_content" }  selector 可选，不传则自动检测
      ...
      if (rawBody) {
        try {
          ...
          // 非 JSON，当作纯 CSS 选择器字符串
          selector = rawBody.trim() || null;
        }
      }
      const selectorStr = selector ? JSON.stringify(selector) : "null";
      const js = EXTRACT_FN + "(" + selectorStr + ")";
      const resp = await sendCDP(
        "Runtime.evaluate",
        {
          expression: js,
      ...
      res.end(
        JSON.stringify({
          ...
          endpoints: {
            ...
            "/extract?target=": "POST body=JSON{selector} - 提取正文为 Markdown",
