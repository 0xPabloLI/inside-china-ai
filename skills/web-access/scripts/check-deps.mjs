...
async function ensureProxy(expectedBrowserId, browserOverride) {
  ...
  if (health?.status === "ok" && health.connected) {
    ...
    if (
      ...
      console.log(
        `proxy: 浏览器不一致 — 当前已连着 ${runningLabel}，但本次需要 ${expectedBrowserId}`,
      );
...
function printAvailableHint(detected) {
  ...
  if (detected.length) {
    console.log(
      `  已开启远程调试：${detected.map((b) => `${b.label} (${b.id}, port ${b.port})`).join("、")}`,
    );
  }
...
async function resolveAndReport(override) {
  ...
  switch (result.kind) {
    case "ok": {
      ...
      console.log(
        `browser: ok (${result.browser.label}, port ${result.browser.port}) ${sourceTag}`,
      );
    ...
    case "ambiguous": {
      ...
      console.log(
        "  请询问用户：哪个浏览器作为 Agent 的默认？（写入 config.env 的 WEB_ACCESS_BROWSER）",
      );
    ...
    case "mismatch": {
      ...
      console.log(
        `browser: error — ${sourceDesc}的浏览器是 "${expected}" (${expectedLabel})，但没连上`,
      );
      console.log(`  Agent 处理顺序：`);
      console.log(
        `    1. 先用系统命令打开 ${expectedLabel}（按你所在平台自行选择，如 macOS 的 open -a），再重新运行 node check-deps.mjs`,
      );
      console.log(
        `    2. 若仍报相同错误，可能是因为远程调试开关没启用 —— 告诉用户：在 ${expectedLabel} 的地址栏访问 ${expected}://inspect/#remote-debugging，勾选 "Allow remote debugging for this browser instance"`,
      );
      printAvailableHint(result.detected);
      if (result.source === "preference") {
        console.log(
          `  也可以编辑 config.env 改默认偏好，或本次临时换浏览器：node check-deps.mjs --browser <id>`,
        );
      }
    ...
    case "empty": {
      ...
      console.log(
        `  支持的浏览器：${knownBrowsers()
          .map((b) => b.label)
          .join("、")}`,
      );
      console.log(
        '  在你想用的浏览器地址栏打开 chrome://inspect/#remote-debugging 或 edge://inspect/#remote-debugging，勾选 "Allow remote debugging for this browser instance"',
      );
      return { proceed: false, exitCode: 1 };
    }
  }
