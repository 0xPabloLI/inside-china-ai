...
提取页面正文为干净 Markdown。POST body 为 JSON，`selector` 字段可选。
- 传 `selector` → 精确提取指定 CSS 选择器对应的容器
- 不传或为 `null` → 自动检测常见正文容器（`article`、`main`、`#content`、`#js_content`、`.post-content` 等）

保留标题层级（H1-H6）、代码块、行内代码、链接、图片、列表、引用、表格、分割线。跳过 `script`、`style`、`svg`、`iframe` 等非正文元素。
...
# 也接受纯 CSS 选择器字符串（非 JSON 时按字符串处理）
curl -s -X POST "http://localhost:3456/extract?target=ID" -d 'article.post-content'
```

返回 `{ value: "Markdown 文本" }` 或 `{ error: "错误信息" }`。

> `/extract` vs `/eval`：`/extract` 是浏览器侧 DOM→Markdown 转换，返回的是已清洗的 Markdown 文本，省去模型从 HTML 提取正文的 token 成本。`/eval` 适合精确查询特定 DOM 数据（属性、结构、事件等），`/extract` 适合获取整篇文章/页面正文。
...
| 错误 | 原因 | 解决 |
|------|------|------|
| `Chrome 未开启远程调试端口` | 浏览器未开启远程调试 | 提示用户打开 `chrome://inspect/#remote-debugging` 并勾选 Allow |
