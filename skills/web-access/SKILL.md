...
**⑤ 登录态平台抓取：最小交互** — 每一次页面加载和脚本注入都是风控信号源，把访问面压到与"需要的数据"等长：只读目标数据页，单遍顺序访问，页间拟人间隔，页面渲染完成前保持静默（等待既是风控礼仪也规避渲染冲突），同一登录实例贯穿全程，失败页记录后跳过留给下轮。抓取脚本把这些原则内建为默认行为——可参照 `scripts/short-video/lib/tiktok-video-details.mjs`。
