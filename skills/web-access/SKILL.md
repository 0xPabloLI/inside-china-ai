...
**⑤ 登录态平台抓取：最小交互** — 每一次页面加载和脚本注入都是风控信号源，把访问面压到与"需要的数据"等长。路径选择先行：官方批量导出（CSV/ZIP）能覆盖数据需求时优先于逐页抓取，逐页只抓导出缺失的字段。抓取时：只读目标数据页，单遍顺序访问，页间拟人间隔，页面渲染完成前保持静默（等待既是风控礼仪也规避渲染冲突），同一登录实例贯穿全程，失败页记录后跳过留给下轮。抓取脚本把这些原则内建为默认行为——可参照 `scripts/short-video/lib/tiktok-video-details.mjs`。
...
**权威报告（官方 PDF/白皮书/监管文件）**：网页版和二手总结只是线索，必须下载 PDF 原文并解析目标章节——长文档用 pdfplumber/pypdf 按目录或页码定向提取，不做全文 dump（#266）。「简报省略细节 ≠ 原文未提及」，核实「官方是否说过 X」以原文为唯一判据。

**中英双向对比搜索**：中文线索翻成英文关键词（专名/模型名/作者名）查权威一手源；英文报道翻成中文关键词追中文传播链，定位首发渠道与引述偏差。
