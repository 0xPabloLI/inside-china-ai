...
> **SUPERSEDED** — This spec has been superseded by `docs/spec-vertical-cropping.md`
> (2026-08-26). It is kept here for historical reference only.
> The current code uses `analyze_semantics` + Markdown parse (not `analyze_fit` + JSON
> parse), and `focus` is deprecated in favor of `cropFocus: { x, y }`.
...
- **Python side** (`vlm_analyzer.py`): new `handle_analyze_fit(model, processor, path)`
...
- **JS side** (`visual-analyzer.mjs`): new `analyzeFit(assetPath)` export. Pushes
...
- `analyzeAssets()`: after `describeImage/describeVideo`, check aspect ratio. If
  ...
  Do NOT call `closeVisualAnalyzer()` here (moved to main function's finally).
...
- `closeVisualAnalyzer()` moves from `analyzeAssets()` finally to main function's finally
...
1. **`visual-analyzer.test.mjs`** — test `analyzeFit()` JS API: mock Python subprocess,
...
|------|-------------|------|------------|
| `lib/vlm_analyzer.py` | New `handle_analyze_fit` handler + `analyze_fit` action in main loop | Low | Pure addition. Existing `describe_image`/`describe_video` handlers unchanged. |
| `lib/visual-analyzer.mjs` | New `analyzeFit()` export. `requestQueue` entry format unchanged (already has action + path). | Low | Pure addition. Existing `describeImage`/`describeVideo` unchanged. |
...
| `lib/asset-sourcer.mjs` (`analyzeAssets`) | Add `analyzeFit()` call for landscape assets. Move `closeVisualAnalyzer()` to main finally. | Medium | VLM lifecycle change. If `closeVisualAnalyzer` is not called in `analyzeAssets`, the process stays alive — must ensure main function's finally always runs. |
...
- The VLM process lifecycle change (moving `closeVisualAnalyzer` to main finally) is the
  highest-risk change: if the main function throws before `finally`, the process
  leaks. The existing `process.on("exit")` cleanup in `visual-analyzer.mjs` is the
  safety net.
