...
- [x] `npm run lint && npx tsc --noEmit` 通过（全量 lint + `tsc --noEmit` + `npm run build` 均 EXIT:0，2026-09-01；`render-remotion.mjs` 的 upscale hunk 曾与并行 session 改动同文件，对方部分已先行提交，本 hunk 随补交 commit 入库）
