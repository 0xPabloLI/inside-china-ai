...
- [ ] assignAssetsToScenes 扩展（绑定不 spill、overlap gate、fail-closed、40% 在线贪心）
- [ ] 审计字段（relevanceScore/Source/Reason、reuseStats）
- [ ] 矩阵行 #8/#9/#14/#15/#16/#17/#18/#19/#20/#26 单测 + 回归全绿

## T7 — 编排接线（#138）
- [ ] asset-sourcer main() per-claim 搜索 + fallback 保留 + `--relevance-threshold`
- [ ] main.mjs Step 1.5 不再传 companies[0]
- [ ] 矩阵行 #2/#6/#10/#21 单测（网络 mock）
- [ ] Real Data Smoke（qwen4-preview 链路，不下载不推理）

## T9 — 文档同步 + 归档 + issue 收尾（#139）
- [ ] writing-for-agents 加载判定 → 4 docs + CONTEXT.md 更新（三查通过）
- [ ] spec/tickets/review 归档 docs/archive/
- [ ] GitHub issues 关闭 + roadmap 更新
