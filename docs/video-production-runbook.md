...
- Kaggle 免费层同时只允许 **1 个 GPU session 运行**。多条 `main.mjs` 管线并行时，kernel slug **默认从 content id 派生**（`cosyvoice3-cuda-<pipelineId>`，#267），每条管线自动获得独立 kernelId，**无需手动设置**；`COSYVOICE3_KAGGLE_SLUG` env 可显式覆盖。若既无 env 又无法派生 content id，会 fail-fast 报错（防串台——共用 slug 会互相覆盖 kernel 源码而不是排队）。第二个 kernel push 后进入 `QUEUED` 排队等待前一个结束，这是**预期行为**，不是卡死。
...
### AI Image (T2I) generation — Boogu Image Turbo (#155)
...
**Backend**: mflux's **Boogu Image Turbo** (Boogu `Boogu-Image-0.1-Turbo`, 10B, **Apache-2.0**; CUHK + HKUST, arXiv:2607.13125), run locally on MLX with 4-bit quantization. Measured at **~364 s per 832×1216 image on M2 Pro 32GB**. Selected via 6-model VLM comparison on 2026-09-20 (see `docs/research/local-image-gen-models-2026.md`). Fallback路线 (no separate admission needed — same local-model rule): **Z-Image base** (`mflux-generate-z-image`, CFG=4, `--guidance 4`, `--steps 50`) and SD3.5 sit behind `AI_IMAGE_BACKEND`; both are CFG-capable, which matters for the negative prompt below.

**Negative prompt reality (the load-bearing backend fact)**: Boogu Turbo is guidance-distilled — CFG is disabled, so mflux accepts `--negative-prompt` on this path but never encodes it. The runner therefore passes `--negative-prompt` **only** to CFG-capable backends (`mflux-z-image`, `sd3.5`) and omits it on the Turbo default. The NEGATIVE clauses still ride the composed prompt (`composeImagePrompt` injects them exactly as on the video side), where they act whenever a CFG backend is selected; on Turbo, quality is backstopped by the VLM relevance gate — a bad candidate simply fails the gate and costs a round, which is the accepted trade-off.
