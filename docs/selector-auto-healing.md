...
| 2026-09-24         | polymarket_search                                                | `/search?q=` 302 → `/zh/predictions?q=` 本地化浏览页；首轮定界误判「0 event 链接」（抽样早于 SPA 渲染完成，见第八假判决）；真实形态 = 覆盖锚 `a[href*="/event/"]`、标题在 `aria-label`、锚内 textContent 为空 | registry URL 换 `zh/predictions?q={kw}`，articleScript 按 event 覆盖锚抽 + `aria-label` 标题 + href slug 反推兜底                                                                                                   | `selector-health` **30 条**（relevance guard 过）；56 条 event 锚实测（本 session，同日）                  |
| 2026-09-24         | duckduckgo_search                                                | `html.duckduckgo.com` 对自动化流量（含真 Chrome CDP）弹人机验证挑战页；裸 HTTP 控制组同出口也不通——出口 IP 信誉，非选择器腐烂；#91「lenient, no CAPTCHA」旧认知作废                                       | **标记放弃（RETIRED）**——selector 修不了挑战页；registry notes 记录 RETIRED 依据。检索侧兜底：search-pool（Serper/Brave/Tavily/Jina）与 SearXNG 聚合（内置 DDG 引擎，无需 API key，本机实例当前 Stopped）          | 挑战页截图级证据（CDP 落地正确 URL + 挑战文案）（本 session，同日）                                       |
