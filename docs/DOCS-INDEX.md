...
| `claim-gate-research-2026-09.md`                   | Claim 前强制读评论 gate 调研与裁决（#258）— 业界无 claim-time 上下文完整性标准件；A（claim 脚本）为主 + B（issues.assigned CI 兜底）为辅，B 设计留档暂不安装（触发条件写明）                                                                                                                                                                       |
| `company-relations.md`                             | 中国 AI 公司关系图谱（#248）— 68 实体母子公司/品牌关系；SSOT = `lib/data/company-relations.json`（管线经 `lib/company-relations.mjs` 消费），hashtag/写稿表述/实体色解析规则与认知度分级依据；人读视图                                                                                                                                            |
...
| `ponytail-minimal-code-adoption-proposal.md`       | Ponytail 最小实现规则引入提案 **v6（已结项，2026-09-12 裁决：A-lite 保留 / B-lite 未验证移除 / D 保留）** — 最终落地：A-lite/D → implementation-workflow §6（长期）；B-lite 从 §8 移除（§5 留档）；真相源 = 归档试点记录 `docs/archive/reviews/ponytail-lite-pilot-2026-09.md`（正向证据门槛 + "保留"取代"转正" + hard gate 六项）                                                                                                                     |
...
| `voiceover-pacing-research.md`                     | 旁白语速改革 deep research（2026-09-10，web-deep-research）— EN 140-160 wpm 甜点/写稿优先（删词不拉音节）/TTS 提速上限 1.2×（ElevenLabs Tier 1）/中文 240-300 字/分已是快档/CosyVoice speed=mel 时间拉伸源码实证；Stage 3 写稿规范与 speed 补差决策依据 |
| `hook-vs-narrative-pacing-2026-09.md`              | Hook vs 正文相对语速 + WPM 反馈闭环（2026-09-11，#252 增量调研）— hook 能量峰值不慢于正文（方向性共识，无量化差值）/创作者 170-200 WPM 靠写稿密度非拉速/TTSAudit silence-aware WPM 闭环先例；#252 决策记录与 deepseek-v41-flash-report 真包复测 |
| `vlm-landscape-survey-2026-09.md`                  | 2026 开源 VLM 全市场调研（#262）— MOSS-VL 确认存在但无 MLX 路径（ADR-0009 排除）；新候选 MiniCPM-V 4.6（1.3B）/Gemma 4 E2B/E4B 待同口径实测；现有 Qwen3-VL-2B + GLM-4.1V cascade 维持 |
