...
> v4 → v5 变更（2026-09-04，第三方复核裁决）：§7 门槛改为**需正向证据**（v4 只要求"改变选择/实际触发"，不要求结果更好）；试点表新增交付确认、spec parity、返工、误报、实际收益五类字段；hard gate 从 3 项扩到 6 项（新增 spec parity 缺失、测试被省略或弱化、可访问性）；§5 改动 D 措辞改为按**不变量归属**判断；§2.7 更正独立评测对象（注入的是钩子生成的 full-mode 规则集，不是 33 行内核，**该评测不支持对本项目三条规则的效果外推**）。
...
- JetBrains 结果**方向为正**：成本端证据较强（p=0.004），代码端仅观察到同方向、未达显著。**该评测的注入物是钩子生成的 full-mode 规则集（§2.7），未隔离验证 33 行内核，也未验证本提案的三条改编规则**——因此它只支持"这类规则在本仓库无已知危害"的方向性判断，**不支持任何效果量级的外推**（无依据预期复制 −10~15%）。
...
- **v4 → v5 门槛变更**（2026-09-04，第三方复核 Q1/Q5/Q6/Q7 裁决）：
  - v4 的 A-lite 只要求"改变选择"、D 只要求"实际触发"，**都不要求结果更好**；hard gate 也只覆盖安全与正确性。据此转正会把"规则改变了行为"误当成"规则有价值"。
  - v5 的转正需**正向证据**：A-lite 的改变须伴随正向收益（更少代码/文件/返工且 spec parity 无损）；B-lite 须独有 finding 且误报未造成返工；D 的根因判断须事后被验证为正确；三条都需要 **≥80% 任务的交付确认 evidence**。
