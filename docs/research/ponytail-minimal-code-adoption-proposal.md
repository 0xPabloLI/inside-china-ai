# Ponytail 最小实现规则引入提案（v6，已结项）

> 状态：**试点已结项（2026-09-12 期满裁决：A-lite 保留 / B-lite 未验证移除 / D 保留，结论见归档记录 `docs/archive/reviews/ponytail-lite-pilot-2026-09.md`）**。最终落地：A-lite/D → `implementation-workflow.md` §6 步骤 2（长期规范）；B-lite 从 §8 移除（本提案 §5 留档，复活路径见裁决结论）；试点路由 §9 步骤 3 已删除。**裁决门槛的真相源在试点记录文件**（§7 只保留设计意图，避免两份门槛漂移）。
