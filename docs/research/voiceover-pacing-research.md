...
即 speed 参数对生成的 mel 频谱做**时间轴线性插值**后再 vocoder——与后处理 atempo（波形域 WSOLA）是同一类时间拉伸操作，仅作用阶段不同。这解释了用户在 1.2× 处"native 和外部加速都差不多"的听感。工程含义：

- 无需为"更自然"而强制选 native；两者在 ≤1.2 区间等效
- `speed` 仅支持非流式推理（我们的 kernel 用 `stream=False`，满足）
- mel 域拉伸经 vocoder 重渲染，无 WSOLA 相位 artifact，理论上比波形域更干净——但未做听感盲测验证，不作为选型依据
