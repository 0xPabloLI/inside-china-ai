...
> 变更：v2（2026-09-24）——用户裁决采购环节视频优先；生成形态改为预算决定；新增 D6 视频动效矩阵。**v3（2026-09-24）——读 #301 票评发现 2026-09-14 已裁决媒体优先级链（素材视频→新闻视频→upscale→AI 生成，4a/4b 留后续），D1 链对齐该在案裁决并补新闻视频/upscale 阶梯级；4a vs 4b 的「留后续讨论」由 D1b/D5 承接。**
...
| F9  | 现有相关票：#295 = niche claim→可搜索概念映射（searchHints），**不是**「给所有场景生成 assetNeed」，triage 已裁决 searchHints 于写稿时由 LLM 生成（票评 2026-09-24）；#301 = stock-video-first 路线，**票评 2026-09-14 已裁决媒体优先级链**（素材视频→新闻视频→upscale→AI 生成；4a/4b 留后续），2026-09-15 grilling 并入 #310 转 dormant；#293 = Stage 0 预取；#294 = VLM borderline 二度校验；#298 = T2V 画质评测升级；#310 = 统一素材库 + 收获率 log | GitHub 票面 + 票评（2026-09-24 读取）                                                            |
...
讨论稿候选链（每 media-eligible 场景，从上到下首个可用者胜出；**对齐 #301 票评 2026-09-14 用户裁决的媒体优先级链**）：

```
1. 认领采购（VLM 门控）—— stock video 优先；无合适视频 → stock image（自带 ken-burns 动效）
2. 外部新闻视频（CDP/yt-dlp 域，#314 方向）
3. 素材库复用（#310 统一素材库 + 收获率 log；低质素材走 upscale 提质）
4. AI 生成 —— 形态由速度/预算决定（不固定位次）：
   默认快路径 T2I + ken-burns（6 维 prompt）↔ 预算与场景语义允许时 T2V（8 维 prompt）
   （即 #301 裁决留待后续的 4a vs 4b 问题，由 D1b/D5 承接）
5. 纯版式（CSS 卡片——合法结果，不是失败）
```

- **D1a（已裁决）**：采购环节内视频优先——#301 票评 2026-09-14 用户裁决在先（「有视频就选视频，不选图片；AI 生成是 fallback 的 fallback；当前 ai-image 策略是 Wan1.3B 质量差的 workaround」），v2 会话用户再次确认。开放子项：per-visualType 微调（data/stat-reveal 类版图场景是否也视频优先；hook 已有专属 gates）。
- **D1b（v2 重构，v3 定位为 #301 遗留问题的答案）**：生成级不再是「T2V 先于 T2I」的固定位次，而是**预算驱动的形态选择**——D5 的预算机制同时充当形态选择器。这回答了 #301 裁决明确留白的「4a 直接生成视频 vs 4b 生成图像再做成动效」。
...
| 票                        | 内容                                                                                               | 关系                                                                                                                                                                  |
| ------------------------- | -------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **#354（A）认领供给泛化** | voiceover → assetNeed + searchHints 派生；字段降级为覆盖；存量内容策略                             | 吸收 **#295**；关联 #293（Stage 0 预取接供给）                                                                                                                        |
| **#355（B）统一素材兜底链** | 通用递降链替代五值策略；承接 #301 票评 2026-09-14 媒体优先级链裁决；采购视频优先 + 生成形态预算选择（D1b/D5）+ 动效内容感知矩阵（D6 完整版） | 衔接 **#301**（pixabay-video 源、对 video 放宽阈值、全屏 vs overlay 作为呈现规则输入）、**#310**（复用阶梯级）、#299（hook 专属策略）、#294（VLM 二度校验强化采购级） |
| **#356（C）无主供给退役** | 兜底池/缓存趋势图认领化或退役                                                                      | #297 直接后续；依赖 #354 的派生机制                                                                                                                                      |
| **#357（D6 快修）视频默认 zoom 退场** | 非 hook 视频默认不再用 1.3→1.0→1.15 容器缩放（none vs fade 样张裁决）；内容感知矩阵完整版留 #355 | bug，独立快赢；证据与验收见票正文                                                                                                                                        |
