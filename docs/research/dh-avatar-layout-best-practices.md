...
另一个关键实践（「3 秒法」，多源）：AI 数字人连续出镜 3–4 秒内可信度最高，超时僵硬感累积、掉留存。这与我们「按 scene 可选声明、先 hook/CTA 试点」的既定决策互洽——单 scene 时长 5–15s 处于边界，spec 应写明「数字人段内允许 b-roll/图表切出，数字人不必满段在屏」。
...
4. **「3 秒法」**：AI avatar 连续出镜 3–4 秒（hook 建立可信度）后切 b-roll 是常见留存策略；AI 头部长时间静止/僵硬是掉留存主因 [10][11]。对管线的含义：数字人 scene 的版面应支持「数字人在屏 ⇄ 素材满屏」的段内切换，而非强制满段在屏。
...
- **「数字人根本不该常驻」**：3 秒法支持者认为 AI 头任何超过几秒的连续出镜都在消耗留存 [10]。我们的对冲：数字人声明按 scene 可选、段内可切出；试点先 CTA（时长短、承诺强）。
