# Widget Data: deepseek-talent

> Exported from `src/components/widgets/deepseek/data/people.ts`
> Widget type: 人才流动（talent flow）— DeepSeek 核心成员离职去向
> Last updated: 2026-07-28 (git: 2026-07-28 18:53:54 +0800)
> View component: `src/components/widgets/deepseek/talent-view.tsx` → `TalentView`

## Data

### People (5 entries)

| Name (ZH/EN) | Role | Tech Area | New Company | Departure | Salary | Salary Known? | Note |
|-------------|------|-----------|-------------|-----------|--------|---------------|------|
| 罗福莉 / Luo Fuli | V2 Core Contributor | Base Model | 小米 / Xiaomi | 2025 | ¥10M+/yr (~$1.4M+/yr) | ✅ | 雷军亲自挖角 |
| 王炳宣 / Wang Bingxuan | First-Gen LLM Core Author | Base Model | 腾讯 / Tencent | H2 2025 | ¥10M+ (est.) | ❌ | Based on industry reports |
| 郭达雅 / Guo Daya | R1 Core Researcher / Coder / Math | Reasoning | 字节跳动 / ByteDance | Mar 2026 | ¥10M+ (est.) | ❌ | Seed Team Agent Lead |
| 魏浩然 / Wei Haoran | OCR Series Core Author | OCR | 百度(疑似) / Baidu (suspected) | Around CNY 2026 | ¥3-5M (est.) | ❌ | Inferred from 36kr |
| 阮翀 / Ruan Chong | Multimodal Core Researcher | Multimodal | 元戎启行 / Deeproute.ai | Unspecified | ¥3-5M (est.) | ❌ | Autonomous driving company |

## Sources

- No explicit `sourceUrl` field in data.
- Data sourced from industry salary reports and media coverage:
  - 36氪 (36kr) — Wei Haoran's departure inferred from coverage
  - Industry薪酬报道 — Salary estimates for Wang Bingxuan, Guo Daya, Wei Haoran, Ruan Chong
  - Public media reports on Luo Fuli's recruitment by Lei Jun (Xiaomi)

## Related Articles

- Not embedded in any published article via `<!-- widget:deepseek-talent -->`.
- Content relates to `deepseek-art-of-restraint` (DeepSeek talent drain article section).
