...
**文本溢出根治 epic**（spec：`spec-text-overflow-hardening.md`；2026-09-02 补录 tracker）。T1–T7 已交付；T8–T12 状态如下。方向修正见 spec 决策 63–72（2026-09-02 实施审计驱动）。
...
| #149 | T8: highlight 结构化 {field,text} + 17 处迁移 | — | content/*/scene-data.mjs, verify 管线 | ready-for-agent；**并行 session 进行中**（2026-09-03 工作树 WIP：substring-highlight.tsx + 4 包 scene-data），勿另开 session |
| #150 | T9: media-overlay 补齐 action/context + s9 布局修正 | #175（决策 65：badge 阻断是 T9 新首项，需 TextGate 生产路径先就位） | scene 组件, scene-rules.mjs | ready-for-agent；**并行 session 进行中**（2026-09-03 工作树 WIP：text-slots badge 宽度 + scene 组件），勿另开 session |
...
**Working-tree WIP（并行 T8/T9 session，2026-09-03 确认进行中）**：工作树有一批活跃的未提交改动，内容与 T8/T9 对应——`text-slots.mjs`（stacked-cards badge 宽度，决策 65）、`remotion/src/components/substring-highlight.tsx`（T8 highlight 结构化）、`HookScene.tsx`/`NarrativeScene.tsx`/`text-gate.tsx`/`scene-gate-fixture.tsx`/`types.ts`、`content/{qwen4-preview,doubao-work,light-society,zhipu-glm6-self-training,_gate-smoke}/scene-data.mjs`（T8 17 处 highlight 迁移）及配套测试。该 session 还已推送 c478935/c5d38b4（T10 review nits + handoff）。**本轮（2026-09-03 第二 session）已避开这些文件**；T8/T9/T11 不另开 session 重复做，等并行 session 提交后从 tracker 状态接手。其余未提交项（数字人 leaptalk `leaptalk_inference.py`、`docs/research/digital-human-test-progress.md`、`asset-sourcer.mjs`/`claim-keywords.mjs` 小改、`content/didi-robotaxi-r2/` 新包、hallo3-test）属内容/研究实验，未在 tracker 建 issue，处理前先与用户确认归属。
