...
| #90 | MCP to API migration (Bigsong) | — | source-registry.mjs, search-sources.mjs | lib/bigsong-api.mjs 直接 HTTP 调用. **Matrix/issue 也涉及 `search-sources.mjs`**。⚠️ **2026-09-03 本 session 跳过**：Bigsong API 未经事实双源核实（代码库无引用、官方文档未查），proposal-review 门禁停报；如开工先完成核实 |
