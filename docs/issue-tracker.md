...
**Working-tree WIP（2026-09-03 第三 session 更新）**：原并行 T8/T9 session 的工程改动已全部提交并推送（T8 `2a0b86a`，连同 #153 代码交付 `0eafde2` 与 lint/docs 批次 `872c7d7`/`6779353`/`a6ea7ff`/`d122756`/`e0e97ae` 一并推至 origin main）——`text-slots.mjs`、`substring-highlight.tsx`、`HookScene.tsx`/`NarrativeScene.tsx`/`text-gate.tsx`/`scene-gate-fixture.tsx`/`types.ts`、5 包 `scene-data.mjs` 及配套测试不再 WIP，#149/#150 状态见 Tier 1。工作树剩余未提交项（数字人 leaptalk `leaptalk_inference.py`、`docs/research/digital-human-test-progress.md`、`docs/video-script-writing-guide.md`、`asset-sourcer.mjs`/`claim-keywords.mjs` 小改、`content/didi-robotaxi-r2/` 新包、hallo3-test）属内容/研究实验，未在 tracker 建 issue，处理前先与用户确认归属。
