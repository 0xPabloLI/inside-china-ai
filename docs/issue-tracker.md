...
| #176 | Remotion 渲染 mp4 音频轨 ~93ms 前导静音 → 真实音画/字幕偏移超容差 | —（根因 1 验证器错代音频 `bac686e` 已修；本票为根因 2） | `lib/render-remotion.mjs` / Remotion 音频编码 / `post-process.mjs` | **2026-09-02 新建**。修好音频同步验证器后暴露的真实渲染层 bug：`_gate-smoke` 9 scene 全部精确 +93ms（scene1 0.09 vs 0.00… 一律 +93ms）。`ffmpeg silencedetect` 证实成片音频轨**开头 93ms 真实前导静音**（源 mp3 无前导静音）；三次历史渲染（09-01 至 09-02）全 86–93ms 确定性常量；音频流 `aac @96000, start_pts=0` → AAC 编码器 priming（mp3→Remotion 解码→AAC 重编码 double-encode）。字幕按 timeline 0.00 烧入视频帧无该偏移，故字幕比音频早 93ms，超 `AUDIO_SYNC_TOLERANCE=0.08` 13ms → 校验 FAIL。**影响所有该管线产出短视频**。修复方向：确认 93ms 来自 Remotion raw 还是 post-process；剥离 AAC priming（`-af aresample=async=1` / 去 edit list / 喂 WAV 避免 double-encode）；修复后 `audio-sync.test.mjs` + `_gate-smoke` 实跑回归 |
...
| **Video Pipeline** | #98, #99, #100, #101, ~~#113~~ ✅, #35, #32, #75, #127, #29, #155, #156, #157, #158, #166, #141, #149, #150, #151, #152, #175, #153, #176 | W1, W2, W4, Dormant, T3（文本溢出 epic #141/#149–#152/#175 属视频渲染质量线；#153 为其 preflight 存量项；#176 为渲染音频 AAC priming 偏移） |
