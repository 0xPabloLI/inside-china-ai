...
| **W2 — 搜索扩展与路由** | `source-registry.mjs` 增源 + `search-sources.mjs` 路由 + `cdp-client.mjs` fallback | ~~**#64**~~ ✅ closed (acf9dd8 已推送) · ~~**#66**~~ ✅ closed (bc733bd/ff29ebc 已推送) · **#90** (T2) Bigsong API（⚠️ 本 session 跳过，需人工决策）· **#97** (T2) WeChat RSS · ~~**#91**~~ ✅ closed (2026-09-03, commit `653e508` — #112 hard dep 解除) · **#112** (T2) image search pool · **#75** (T2) 视频源标注+下载方案（从 W4 提升） · ~~#122~~ ✅ closed · ~~#123~~ ✅ closed · ~~#124~~ ✅ closed · ~~#125~~ ✅ closed | #66 unblocked（#67 ✅）；#65 依赖 #64/#90；#91 依赖 #89 P0（**从 W3 前移到 W2**：#112 显式依赖 #91 DDG infra，不能在 #112 之后）；#109 已合并进 #65；#112 依赖 ~~#91~~ ✅(2026-09-03)/#103 ✅/~~**#115 (hard)**~~ ✅ — **hard blockers 全部解除**；#75 依赖 #54 done | #64/#90/#66 共享 registry/search collector，按 Matrix 串行；#97/#112 可并行；#75 改 source-registry + asset-sourcer，与 #64/#66 串行（~~#88~~ ✅ done，不再约束） |
...
| **W4 — 延后视频链 + 独立增强** | 视频渲染 P5-P8b 线性序列 + 独立研究/增强任务 | **#98** (T3) P5 ASR · **#99** (T3) P6 timeline · **#100** (T3) P7 cache · **#101** (T3) P8b focus · **#85** (T3) Bloomberg · **#94** (T3) visual intent · **#108** (T3) free inference · **#117** (T3) currency conversion · **#140** (T3) anti-bot follow-up (#89 剩余 P1/P2/P4–P7 切片) · **#166** (T3) B-roll 常量注入第二层（触发条件：b-roll 用量 10+ scene） | #99 依赖 #98；#101 依赖 #69 推荐接 #100；#166 无 hard blocker，但触发条件未满足前不建议开工 | 视频链按 P5–P8b 显式 Sequence 推进；独立增强受各自 Matrix 约束。**#101 不是 #94 的 child**——是 P5-P8b 线性序列中的 P8b（见 Tier 3） |
...
| `asset-sourcer.mjs` | ~~#88~~ ✅, ~~#114~~ ✅, #75, ~~#115~~ ✅, ~~#128~~ ✅ | 🟡 中（#84 已 merge，搜索缓存已就位；#110 ✅ done；#75 视频源下载+capability 标注；#114 ✅ SVE done；#115 ✅ downloadCandidate 提取 done；#128 SVG filter may touch isLogoOrIcon） |
...
| `vlm_analyzer.py` | ~~#113~~ ✅, ~~#127~~ ✅ | ✅ 无活跃 issue（#127 已于 2026-09-03 关闭——`deep_analyze()` 抽取完成；#113 图片预处理更早已 CLOSED） |
| `scene-rules.mjs` / `remotion/src` 场景组件 | #94（可能）, #155, #166 | 🟢 低——#166 第二层会同时碰 `scene-rules.mjs`（改校验语义）与 `lib/b-roll/*`（改注入），与 #155 同处此行，开工前需确认串行 |
