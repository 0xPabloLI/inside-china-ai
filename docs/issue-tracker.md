...
**文本溢出根治 epic**（spec：`spec-text-overflow-hardening.md`；2026-09-02 补录 tracker）。T1–T10 已交付；仅剩 T11（#152），状态如下。方向修正见 spec 决策 63–72（2026-09-02 实施审计驱动）。

| # | Issue | Blocked by | Conflict files | Notes |
|---|-------|------------|---------------|-------|
| #141 | Spec parent: 短视频文本溢出根治 + 时间轴对齐 | — | — | Spec 定稿于 `spec-text-overflow-hardening.md`。tickets #142–#150（T1–T10 已交付）已随交付关闭；**仅剩 T11（#152）** |
...
| #152 | T11: 端到端重渲染 + 存量清单 + 文档 + 归档 | #175（决策 71） | verify-video.mjs, docs | ready-for-agent；**下一 session 首选候选**（前置 #153 已关闭解除；含 #153 遗留 slotCharBudget 重构，text-slots.mjs 现已无并行冲突） |
