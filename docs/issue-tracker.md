...
| # | Issue | Blocked by | Conflict files | Notes |
|---|-------|-------------|---------------|-------|
| ~~#66~~ | ✅ Scenario-driven fetch layer + API→CDP fallback check | ~~#67~~ ✅ done | ~~search-sources.mjs, cdp-client.mjs, source-registry.mjs, asset-sourcer.mjs~~ | ✅ **2026-09-03 交付并关闭**（commits `bc733bd`/`ff29ebc` 本地待 push）：① `fetchPage` 四层（auto/static/jina/cdp）；② Step 0.5 `shouldSkipCdpOnApiFail`（issue 两允许路径中的 `cdpUrl === apiUrl` 检查）。TDD 23 red→green；全量 2776/2777（verify-lfs flake #188 与本票无关）。全文提取接线拆票 #187 |
...
| ~~#64~~ | ✅ Add free API sources + baidu_news + reclassify Currents/Noozra | — | ~~source-registry.mjs, search-sources.mjs~~ | ✅ **2026-09-03 交付（代码完整）**（commit `acf9dd8` 本地待 push）：① baidu_news 与 google_news 同模式（CDP news.baidu.com/ns，无账号/key）；② currents/noozra_search 移入 INTERNATIONAL_SOURCES（有 gnews 先例，免同步消费方）。TDD 5 red→green，定向 10/10。**真实数据冒烟不可用**——本机 Chrome 会话被百度反爬阻断（SPA 20s 不渲染），如实报告；真实采集待正常 cookie 会话复测 |
...
| ~~#127~~ | ✅ VLM Cascade Router: Qwen3-VL-2B fast path + GLM-4.1V-9B deep analysis fallback | — | ~~vlm_analyzer.py~~ | ✅ **2026-09-03 关闭**（按推荐路线先抽 `deep_analyze()` 再 close，commit `fc40dae` 本地待 push）。`run_vlm_inference()` 共用 seam（视频抽帧/图片 crop+resize + 临时清理）+ `deep_analyze()`（escalated 标记 + sourceMode），`handle_analyze_semantics` ~90 行 2B/deep 重复预处理块消除；新增 `__tests__/test_cascade_inference.py` 8 项 TDD（red→green）。验证：python 36/36 + Node `visual-analyzer.test.mjs` 38/38 + 管线 venv 真实 smoke 两条（unitree-building.jpg 2B 快路径识别正确；纯色图逼出升级信号 → RAM 门控优雅回退 2B——升级+回退契约的真实运行证据）。GLM 完整推理链未重跑（RAM 门控），该路径逻辑未变且有 2026-08-30 端到端 `escalated: true` 证据。Router 第 4 信号「>1920px+other」代码中从未实现，#113 后失去意义，superseded。证据评论见 issue。历史：2026-08-30 核实级联已实现跑通（should_escalate 16/16、check_ram 5/5） |
...
| ~~#128~~ | ✅ SVE follow-up: Logo/Icon SVG filter + 4 deferred design points | — | ~~extract-media.mjs, asset-sourcer.mjs (may), source-registry.mjs (may)~~ | ✅ **2026-09-03 closed**（commit `2c34638` 本地待 push）：item 1 SVG data URI bug 修复（extract eval `data:` 排除 + `LOGO_ICON_REGEX` 加 `data:image`，双保险）+ item 4 `CDP_IMAGE_CAPABILITIES`→`CDP_MEDIA_CAPABILITIES` 重命名。item 2/3 按 issue 自述设计维持（不需）；item 5 延后至 #75 落地有消费需求再实现。2 tests red→green；全量 2750 passed。证据见 issue comment |
...
| ~~#177~~ | ✅ prettier 3.9.6（node_modules）vs ^3.7.3 pin — 1169 既有 prettier findings | — | ~~package.json（或全量 reformat）~~ | ✅ **2026-09-03 交付（部分）**（commit `bf6bea2` 本地待 push）：prettier 精确锁 3.9.6（根因杜绝再漂移）+ npm install 重装；lockfile 顺带修复 vite-tanstack-config 2.12.0→2.13.1 desync（范围混入，已向用户报告）。**lint-clean 目标未达成**——锁定 3.9.6 后仍有 1190 条 prettier findings（已提交文件并非全由 3.9.6 格式化，ticket 前提部分证伪）；全量 reformat 已被用户否（diff 大），余项处置留待后续决策 |
| ~~#178~~ | ✅ docs-lint: tracker Last-inventory 轮换行豁免 writing-for-agents gate WARN | — | ~~lint-doc-hierarchy.mjs + 测试~~ | ✅ **2026-09-03 closed**（commit `4be7963` 本地待 push）：「替换且同 diff 存在新 Last inventory 行」豁免、净删除仍 WARN，逐行豁免不吞同 diff 其他结构改动。TDD 3 red→green；38/38 测试 + lint:docs PASS + diff --check 干净 |
...
| #171 | Lint script cleanup: 6 judgement-call findings from #170 review | ✅ **2026-09-03 closed**（commit `2c5b3d9` 本地待 push）：核对发现 3 项已随 `eeab5ec`（#129 交付）解决（#1 模板去重、#3 重命名 AGENT_DOC_POINTER_CHAIN、#6 isExcluded 接入）；本 commit 补齐剩余 3 项——git 命令模式显式测试、`messageOf()` 断言 helper、`DiffLine` JSDoc typedef。35/35 tests + 脚本本体 PASS。R1（测试+注释无行为变化）。证据见 issue comment |
...
| #64 | 加源 baidu_news + Currents/Noozra 重分类 | ✅ **2026-09-03 closed（代码完整，真实数据待复测）**（commit `acf9dd8` 本地待 push）：baidu_news CDP 源 + currents/noozra 重分类入 INTERNATIONAL_SOURCES。TDD 5 red→green，定向 10/10；真实数据冒烟被反爬阻断，待正常 cookie 会话复测。详情见 Tier 2 行 |
...
| #178 | docs-lint: tracker Last-inventory 轮换行豁免 writing-for-agents gate WARN | ✅ **2026-09-03 closed**（commit `4be7963` 本地待 push）：同 diff 存在 del+add 配对的 "Last inventory:" 行豁免（mechanical re-inventory 非创作决策）、净删除仍 WARN；逐行豁免，同 diff 其他结构改动仍正常 WARN。TDD 3 red→green；38/38 测试 + lint:docs PASS + git diff --check 干净。详情见 Tier 3 行 |
| #177 | prettier 3.9.6（node_modules）vs ^3.7.3 pin — 1169 既有 prettier findings | ✅ **2026-09-03 closed（部分交付）**（commit `bf6bea2` 本地待 push）：prettier 精确锁 3.9.6（根因防止再漂移）+ npm install 重装（顺带修复 vite-tanstack-config lock desync，范围混入已报告）；lint-clean 未达成——锁定后仍有 1190 条 prettier findings（前提部分证伪），余项待用户决策。详情见 Tier 2 行 |
