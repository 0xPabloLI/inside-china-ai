...
**Working-tree WIP（uncommitted, 2026-09-02 晚间）**：自 `8786eb3` 起工作树出现未提交批次（12 modified + 6 untracked），工程相关部分疑似 T9/T10/T11 续作——`text-slots.mjs` 新增 `hook.hero-center.subtitle`/`subtitleHighlight` 宽度 820（决策 65 测量）；`HookScene.tsx` bigNumber `expectAnnotation` + `whiteSpace:nowrap` + 长数字裸显回退；`visuals.tsx` Slot 支持 `left`/`right`；`CtaScene.tsx` hero/support 垂直居中覆写 + logo 尺寸；`ContextScene`/`NarrativeScene`/`ShortVideo` 接入 `MediaBackground`（`contentDir`）。这些改动**未提交、未跑门测试、未验证**，#149–#152 状态仍为 `ready-for-agent`。其余未提交项（数字人 leaptalk `leaptalk_inference.py` +546 行、`articles/zhipu-glm6-self-training.md`、两处 `content/*` 目录、`f5-mlx.mjs` 离线化、`asset-sourcer.mjs` meta.mjs 回退）属内容/研究实验，未在 tracker 建 issue。**下一步**：决定这批工作树改动的归属与提交策略（#176 已于 2026-09-03 独立交付，未触及本批 WIP 文件）。
