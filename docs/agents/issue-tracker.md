...
- **Pickup queries filter `--state open`**: state and signal labels can linger on closed issues (2026-09-19 audits: 11 closed issues carried `ready-for-agent`, 9 carried `needs-triage`/`ready-for-human`, 9 carried `P1`–`P3`, 33 carried `comments-unread`; all swept same day). Pick work only from `gh issue list --state open ...` — a closed issue is never a pickup candidate regardless of its labels.
