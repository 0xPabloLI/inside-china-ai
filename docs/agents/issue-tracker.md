...
- **Pickup queries filter `--state open`**: state labels can linger on closed issues (2026-09-19 audit: 11 closed issues still carried `ready-for-agent`; swept same day). Pick work only from `gh issue list --state open ...` — a closed issue is never a pickup candidate regardless of its labels.
