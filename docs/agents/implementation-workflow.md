...
2. **bug 任务先执行根因搜索**：搜索相关定义与所有调用方；仅当该不变量属于公共 seam 时在 seam 处修复根因，否则在各调用方分别修复。随后按选择顺序决定实现路径——在读完被改动代码的真实调用流程、并满足已确认行为和验证义务的前提下，依次考虑：无需新增代码、契约匹配的现有实现、标准库或平台原生能力、合适的已装依赖、最小自定义实现。若需新增代码，选择最高、最稳定的 public seam；seam 本身是设计问题时调用 `codebase-design`。不得用少行替代正确性、信任边界校验、防数据丢失的错误处理、安全、可访问性或测试。
...
所有代码改动至少运行 affected tests。已知环境性失败（隔离重跑即绿、缺本地工具、node 版本漂移）见 `docs/conventions/test-env-baseline.md`——解释失败前先对照该清单。按影响触发以下检查：
...
- §6 步骤 2 的选择顺序（A-lite）与根因搜索（D）：依据与上游证据见 `docs/archive/reviews/ponytail-lite-pilot-2026-09.md`（2026-09-12 期满裁决「保留」，长期规范）。§8 原 B-lite stdlib/native 重复检查经同一裁决为「未验证」已移除（`docs/research/ponytail-minimal-code-adoption-proposal.md` L2 留档）。
