...
| **Phase 2: 检索体系与运行环境固化**                  | ✅ **#292** search-sources 体系性重审与通用/专用分流 (P1, 交付·open)<br>✅ **#287** 管线入口统一加载 .env.local (P1, 已闭票)                                                    | → **#275** (全链路不自载缺陷, 由 #287 闭环)<br>→ **#285** (needsAuth 403 探针放宽, P2)<br>→ **#284** (Route C 循环引用消除, P2)<br>→ **#276** (web-deep-research 路由接入, P2)<br>→ **#286** (trend 模式相关性护栏, P2)<br>→ **#281** (Search Pool 引擎可用性复测, P2)<br>→ **#269** (死源自动修复 Phase 2, P2)<br>→ **#231** (环境固化真机验收, P3)                                                                                                            | #292 主导搜索链路宏观分流架构（专用源直连平台 MCP vs 通用源进 Search Pool L3 兜底，吸收关闭 #283）；#287 主导环境变量单一入口加载契约，闭环 #275。                                                                       |
...
| **W2B**     | **Phase 2: 搜源架构重审与分流落地**                                 | `scripts/short-video/lib/search-sources.mjs`<br>`skills/search-pool/SKILL.md`<br>`skills/web-access/`    | ✅ **#292** (search-sources 体系性重审与通用/专用分流, **T1**, 交付·open——终版：x_search 平台保真不进 pool)<br>**#285** (login-gated 站点 403 探针放宽, **T2**)<br>**#284** (Route C 循环引用消除, **T2**)<br>**#276** (web-deep-research 接入 Search Pool CLI, **T2**)<br>**#286** (trend 模式相关性护栏, **T2**)<br>**#269** (死源自动修复 Phase 2, **T2**)<br>**#231** (TTS/渲染环境固化真机验收, **T3**) | 依赖 W2A 的 #287 确保环境变量正常；#292 为搜索分流大盘票（吸收 #283），**已交付**——平台保真链与 pool 六源资格为终版裁决；#285 依赖 #292 结构。                                                                                                                                                                                       | ⚠️ `search-sources.mjs`（#292, #285, #307）高冲突须串行；技能文档（#284, #276）可独立并行。                              |
...
- **划分波次**：
  ...
  - **Wave 2B（搜源分流架构与去环落地）**：✅ **#292**（专用源直连平台 MCP vs 通用源 L3 Pool 分流总图，吸收 **#283**，交付·open——终版裁决：平台保真链不进 pool，pool-eligible = 6 通用源）+ **#285**（needsAuth 403 探针放宽）+ **#284**（Route C 循环引用消除）+ **#276**（web-deep-research 接入 Search Pool）+ **#286**（trend 模式相关性护栏）+ **#269**（死源自动修复 Phase 2）+ **#231**（Kaggle wheels 挂载真机终验）。
...
| **#292** | search-sources 体系性重审与通用/专用分流    | Phase 2 · **W2B** | 🎯 Dominant | 交付·open         | P1 enhancement：专用源平台 MCP vs 通用源 L3 Pool，已吸收 #283；审计 + 同日 challenge 终版裁决——x_search 平台保真链不进 pool，pool-eligible = 6 通用源；票 open 待用户复核闭票。 |
...
- **裁定规范**（用户裁决，2026-09-15）：
  - **平台保真链**：平台专用源（x_search/xhs/sogou_weixin/weibo_hot/bilibili 等）全链只用平台忠实层——x_search = `CDP → googleSiteFallback(site:x.com) → Bigsong 直连`，**不进通用 pool**；triage 规则 1 精神优先于 #65 的七源字面名单（该名单写于 #90 平台化之前，已过时）。
