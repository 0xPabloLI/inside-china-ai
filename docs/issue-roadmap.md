...
| **W3C** | 画面适配与智能写稿 | `scene-data.mjs` `lib/visual-analyzer.mjs` | #289 #299 #294 #295（并入 #354）#253 #244 #256 #263（多为 HITL） | 依赖 W3A 选型（#298）与 W3B 素材流 |
...
| #295 | assetNeed 泛化：claim→搜索概念 | P3·W3C | 并入 #354（2026-09-24 提案），待覆盖关闭 |
...
| `asset-sourcer.mjs` | #294 #299 #354 #355 #356 #357（D：#301） | 🟡 素材匹配评分流水线，串行；#297 fail-closed 已闭票（E2 `1b68276` 单模式化）；新组票全部落此文件，严禁并行 |
