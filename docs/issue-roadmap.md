...
> **Last inventory**: 2026-09-15（W2B 开工：🎯 #292 Dominant 交付——pool 触发语义重审 + x_search 经 Bigsong 桥进 pool，W2B 剩余卫星票）
>
> - **#292 交付**（W2B 🎯 Dominant，Tier 1）：search-sources 体系性重审——对照 triage 裁决逐条核实：Step 3 路由（pool 先于 Grok MCP、平台专用源直走专用 MCP）与 toolName 动态判定**已符合裁决**（测试已锁），真正缺口是 x_search：其 Grok 访问为直连 Bigsong apiFallback（#90；用户裁定 **Bigsong ≡ mcp-search-bridge 后端**）且无 mcpFallback 配置 → 旧 `isPoolEligible` 判否，全链空后 pool 永不触达；且 Bigsong 占 Step 2.5 位于 pool 之前，违反 #65「额度引擎先于 unlimited Grok」。修复：`isPoolEligible` 识别 apiFallback（真实 registry 校验精确命中 #65 七源名单）+ `collectFromSource` 重排为 CDP → googleSiteFallback → **pool** → Bigsong/Grok MCP；registry 级不变量测试锁死「仅 x_search 携带 apiFallback」（review Spec 轴 P1）。真实数据 smoke：真实 x_search 源 CDP/googleFallback 0 → pool Tavily 命中 13 条（旧代码该链永远到不了 pool）。Session-Id `20260915-pool-before-grok-29496e`，commits `d34b207` + `1b571ac`。**附带发现：brief-builder 测试日期衰减缺陷**（fixture 硬编码 2026-08-15 + 30 天真实时钟过滤，2026-09-15 起恒挂，与本票无关，待开票）。**W2B 无剩余 Tier 1，下一 frontier = W2B Tier 2 卫星票（#285/#284/#276/#286/#269，#231 T3）。**
...
| **Phase 2: 检索体系与运行环境固化**                  | 🎯 **#292** search-sources 体系性重审与通用/专用分流 (P1, 已闭票)<br>✅ **#287** 管线入口统一加载 .env.local (P1, 已闭票)                                                    | → **#275** (全链路不自载缺陷, 由 #287 闭环)<br>→ **#285** (needsAuth 403 探针放宽, P2)<br>→ **#284** (Route C 循环引用消除, P2)<br>→ **#276** (web-deep-research 路由接入, P2)<br>→ **#286** (trend 模式相关性护栏, P2)<br>→ **#281** (Search Pool 引擎可用性复测, P2)<br>→ **#269** (死源自动修复 Phase 2, P2)<br>→ **#231** (环境固化真机验收, P3)                                                                                                            | #292 主导搜索链路宏观分流架构（专用源直连平台 MCP vs 通用源进 Search Pool L3 兜底，吸收关闭 #283）；#287 主导环境变量单一入口加载契约，闭环 #275。                                                                       |
...
| **W2B**     | **Phase 2: 搜源架构重审与分流落地**                                 | `scripts/short-video/lib/search-sources.mjs`<br>`skills/search-pool/SKILL.md`<br>`skills/web-access/`    | ✅ **#292** (search-sources 体系性重审与通用/专用分流, **T1**, 已闭票)<br>**#285** (login-gated 站点 403 探针放宽, **T2**)<br>**#284** (Route C 循环引用消除, **T2**)<br>**#276** (web-deep-research 接入 Search Pool CLI, **T2**)<br>**#286** (trend 模式相关性护栏, **T2**)<br>**#269** (死源自动修复 Phase 2, **T2**)<br>**#231** (TTS/渲染环境固化真机验收, **T3**) | 依赖 W2A 的 #287 确保环境变量正常；#292 为搜索分流大盘票（吸收 #283），**已交付闭票**——审计确认路由与 toolName 判定符合裁决，修复 x_search 经 Bigsong 桥（≡ mcp-search-bridge 后端）进 pool 并重排 pool 先于 Grok 兜底；#285 依赖 #292 结构。                                                                                                                                                             | ⚠️ `search-sources.mjs`（#292, #285）高冲突须串行；技能文档（#284, #276）可独立并行。                                   |
...
- **划分波次**：
  ...
  - **Wave 2B（搜源分流架构与去环落地）**：✅ **#292**（专用源直连平台 MCP vs 通用源 L3 Pool 分流总图，吸收 **#283**，已交付闭票——路由与 toolName 判定审计确认符合裁决，修复 x_search 经 Bigsong 桥进 pool 并重排 pool 先于 Grok 兜底）+ **#285**（needsAuth 403 探针放宽）+ **#284**（Route C 循环引用消除）+ **#276**（web-deep-research 接入 Search Pool）+ **#286**（trend 模式相关性护栏）+ **#269**（死源自动修复 Phase 2）+ **#231**（Kaggle wheels 挂载真机终验）。
...
| **#292** | search-sources 体系性重审与通用/专用分流    | Phase 2 · **W2B** | 🎯 Dominant | **已闭票**        | P1 enhancement：专用源平台 MCP vs 通用源 L3 Pool，已吸收 #283；审计确认路由符合裁决 + 修复 x_search 经 Bigsong 桥进 pool（pool 先于 Grok 兜底）。 |
...
### 6. 通用/专用源分流与 pool 先于 Grok 兜底（#292）

- **决策背景**：#292 体系性重审发现 pool 进厂语义与 #65 分层原意存在偏差——x_search 的 Grok 访问是直连 Bigsong apiFallback（#90），无 `mcpFallback` 配置导致 pool 永不触达，且 Bigsong 占 Step 2.5 位于 pool 之前。
- **裁定规范**（triage 裁决 + 用户裁定，2026-09-15）：
  - **分流判定**：pool 资格按 fallback 形态动态判定，不加冗余标记——`mcpFallback.toolName === "web_search"`（通用 Grok MCP 桥）或携带 `apiFallback`（直连 Bigsong 桥，**Bigsong ≡ mcp-search-bridge 后端**，用户裁定）。两者是同一 Grok 后端的两种接法。
  - **触发次序**：通用源全链（apiSearch → CDP → googleSiteFallback）为空 → **Layer 3 pool（Serper > Brave > Tavily > Jina）** → Grok 等价物（Bigsong 直连或 Grok MCP）最后兜底；额度引擎永远先于 unlimited Grok。
  - **平台专用源**（xhs/sogou_weixin/weibo_hot/bilibili 等）CDP 失败直走专用 MCP，不进 pool。该架构依赖「仅 x_search 携带 apiFallback」的 registry 不变量（测试已锁死，source-registry.test.mjs）。
