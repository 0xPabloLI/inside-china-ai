...
> **Last inventory**: 2026-09-14（#271 TTS Quality Gate 语义分流与 fail-closed 出片阻断 session）
>
> - **#271 交付闭票**（Wave 1A 的 🎯 Dominant，Tier 1）：Quality Gate 失败按 `failureClass` 语义分流——**pacing 类**（音素正确、仅实测 WPM 偏低）进 #252 补差环，补差仍不合格 → `TTS_PACING_FLOOR_BLOCK` 硬阻断；**acoustic 类**（截断/漏词/相似度低/无音频）禁入补差环、重抽 ≤2 次后 → `TTS_ACOUSTIC_HARD_BLOCK` 硬阻断。两类硬阻断均不受 strict/non-strict 影响。机制见 `docs/content-pipeline.md` → Gate 失败语义分流，交付证据见 issue #271 交付记录。
> - **下一 frontier = #272**（Avatar 唇同步时长一致性 Gate，T1，`ready-for-agent`）：Wave 1A 仅剩此票，出片双保险的 🎯 锚点由它承接。
> - **上一轮（同日下午）**：Wave-Tier 双维执行体系与「深入调研与基准先行」原则落地（commit `c8a45ea`），#283/#290 吸收闭票，Triage Inbox 清零——本轮未改动该架构。
> - **本轮未新增 issue**，无新增重复项。
...
| ---------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Phase 1: 管线可靠性与安全闭环**                    | ✅ **#271** TTS Quality Gate 语义分流与出片阻断 (P1, 已闭票 2026-09-14)<br>🎯 **#270** TTS instruct 标准机制级化 (P2)                                            | → 🎯 **#272** (Avatar 唇同步时长一致性 Gate, P1, **W1A 剩余**)→ **#279** (语速整体上探与耦合, P2)<br>→ **#278** (Hook 文案句式规范, P2)<br>→ **#273** (并行经验与 CDP 并发锁, P2)<br>→ **#274** (Z-Image-Turbo 许可排查, P3)                                                                                                                                                                                                                                    | #271 已交付（Fail-closed 语义分流出片底线）；#270 主导多 TTS 引擎单一来源规范与参数校验；#272 承接 Wave 1A 剩余范围，与已交付的 #271 形成出片双重保险。                  |
...
| ----------- | ------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **W1A**     | **Phase 1: 管线可靠性即时门控**                                     | `scripts/short-video/lib/tts/quality-gate.mjs`<br>`scripts/short-video/main.mjs`                         | ✅ **#271** (TTS Quality Gate 语义分流与出片阻断, **T1**, 已闭票 2026-09-14)<br>🎯 **#272** (Avatar 唇同步时长一致性 Gate, **T1**, W1A 剩余候选)                                                                                                                                                                                                                | 无 hard blocker，即刻可开工。#271 已交付闭票；#272 为 Wave 1A 剩余票，继续阻断脏出片与唇同步 stale。                                                                                                                                                                      | 两票文件不重叠：#271 动 quality-gate.mjs（已闭票），#272 动 main.mjs，可独立开工。                    |
...
- **划分波次**：
  - **Wave 1A（即时门控）**：✅ **#271**（TTS Quality Gate 语义分流与 fail-closed 阻断，已闭票）+ 🎯 **#272**（Avatar 视频有效时长 ≥ TTS 时长强校验 Gate，本波次剩余）。
...
| -------- | ------------------------------------------- | ----------------- | ----------- | ----------------- | ---------------------------------------------------------------------------------------- |
| **#272** | TTS 音频重生成后数字人 avatar 视频不失效    | Phase 1 · **W1A** | 🎯 Dominant | `ready-for-agent` | P1 bug：用户已终审定案方案 B（main.mjs 强校验时长一致性）；承接 Wave 1A（#271 已闭票）。 |
...
| `scripts/short-video/lib/tts/quality-gate.mjs`     | **#270**                               | 🟡 **中**——TTS 质量防线。#271（语义分流与 fail-closed）已闭票；#270（instruct 参数校验与签名拦截）须与已落地的 #271 `failureClass` 分流契约保持一致。              |
...
- **裁定规范**：
  ...
  - 音频与视觉同步（#272，待办）：avatar 视频有效时长 < 当前 TTS 音频时长 → 阻断出片，杜绝唇同步 stale 流入渲染流水线。
