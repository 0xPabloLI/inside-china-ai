...
| **#306** | brief-builder fixture 日期衰减（测试基线恒挂 6 例） | Phase 2 · **W2B** | Satellite   | `ready-for-agent` | P2 bug：fixture 硬编码 2026-08-15 撞真实时钟 30 天窗口，5 例 + `e2e-pipeline` 1 例恒挂。**✅ 已交付**（commit `f7e2731`，`daysAgo()` 相对日期机制，覆盖 `brief-builder` / `claim-auditor` / `e2e-pipeline`，research 148 单测全绿，净化全量验收基线）。待后续合流闭票。 |
...
| **#314** | douyin/xhs 视频素材源——搜索发现入口 + registry 白名单 | Phase 3 · **W3B** | Satellite   | `ready-for-agent` | P2 enhancement：用户方向性确认抖音/小红书应成素材源。**下载层已存在**（#75 批次：`DOUYIN_CDP`/`XHS_CDP`/`XHS_DOWNLOADER`/`REDNOTE_MCP` 四适配器 + selectStrategy URL 路由），缺口 = 搜索发现入口（探针 4 教训：CDP 扫搜索页拿不到播放器）+ `capabilities.videos` 白名单语义（按验证过的端到端能力声明）。 |
...
| **#306** | 2026-09-15 | brief-builder 测试日期衰减：fixture 硬编码 2026-08-15 + 30 天真实时钟过滤，2026-09-15 起恒挂 6 例（#292 review 期间发现，与搜源改动无关） | ✅ **已分流**：Phase 2 · **W2B** / Tier 2 / Satellite（bug）/ `ready-for-agent`。✅ **已交付**（commit `f7e2731` 消除 6 例恒挂，research 148 单测全绿） |
