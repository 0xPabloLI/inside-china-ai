...
| **#285** | login-gated 站点 403 探针误杀面收口（前置探针 + quarantine 复检） | Phase 2 · **W2B** | Satellite   | ✅ **已交付** (2026-09-20，远端闭票待授权) | P2 enhancement：复用 needsAuth 避免知乎等登录源被误杀。**已交付**：前置探针部分由 #317 `f7e18df` 完成；本次收口 quarantine 复检的同源误杀（裸探针判死 → `dead-url-recheck` 每 7 天续押）。seam 落 `lib/source-health.mjs`（`isProbeAuthoritative` / `judgeUrlProbeForSource` / `planQuarantineRecheck`），两个探针调用点共用同一判决；非 needsAuth 源 403/404 仍判死 + 仍 quarantine。needsAuth 复检改为「一份受监督抓取 + 真实结果裁决」，是 triage 裁决未覆盖的 session 自主决策，待用户复核。commits `47fbeaf` / `c957bdc`。                                                                                                            |
