...
**双文件约定（网页类素材）**：从网页 URL 抓取的素材（微信文章、博客、报道）存两个文件，角色不同、互不替代：

| 文件 | 角色 | 消费者 |
| --- | --- | --- |
| `<话题>.md` | 工作格式 — Agent 提取数据、生成文章/scene-data、RAG embedding | Agent + reindex |
| `<话题>.html` | 证据快照 — 抓取时的原始响应，只读存档，永不编辑 | Stage 5a 作为文章附件上传，供读者验证声明 |

Markdown 是唯一可编辑的工作格式；HTML 快照与它**不是**双份维护（快照从不编辑，无漂移风险）。原生文件类素材（用户提供的 PDF/DOCX/CSV）无需 HTML 快照——原文件本身就是附件。
