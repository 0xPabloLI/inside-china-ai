...
核实权威报告（官方 PDF/白皮书/监管文件/法庭文书）时的三条硬规则：

1. **必须拿到并解析原文**。网页版与二手总结只作线索——「简报省略细节 ≠ 原文未提及」。凡核实结论涉及「官方说了/没说」，以原文为唯一判据。
2. **长 PDF 定向提取**。10MB+/百页级 PDF 禁止全文无序 dump：用 pdfplumber/pypdf 按目录或页码定位目标章节做结构化提取。
3. **中英双向对比搜索**。中文线索翻成英文关键词（专名/模型名/作者名）查权威源与一手出处；英文报告翻成中文关键词追中文传播链，定位首发渠道与引述偏差。

> **操作示范（Anthropic 威胁情报报告 2026-09-10，#266 缘起案例）**：首轮只读网页版 + The Hacker News 二手总结，误判「PLA 涉军监控 / 俄国防部凭据 / 公安维稳 / Fable cyber capabilities」等案例为夸大；后下载 154 页官方 PDF，pdfplumber 定向解析 distillation 章节（143-154 页），逐条确认全部准确；再用英文案例关键词回搜中文渠道，定位 X 中文首发帖与传播链。教训：网页版省略具体案例，只读二手材料会把官方细节误判为失实。
...
**TTS instruct（音色/口音）**：scene 的 emotion instruct 取自 `visualType`/`refStyle`，**写稿侧只需选标准风格**——不要写 `scene.instruct` 覆盖、不要写 instruct 文本。风格清单与四维标准（Persona + Accent + Emotion + Pacing）的唯一来源是 `scripts/short-video/lib/tts/instruct.mjs` 的 `INSTRUCT_STANDARD`；风格缺失时 pre-render gate 直接 FAIL，并指明该加哪个条目。机制选型与三层门控的理由见 `docs/archive/handoffs/tts-indian-accent-handoff.md` → 「单一来源与防回归门控（#270）」。
...
视频制作的技术细节（TTS 引擎、渲染参数、文件位置）见 `docs/video-production-runbook.md`。
