...
- **Lockfile**: `scripts/short-video/requirements-video-ai.in` lists direct dependencies; `requirements-video-ai.lock` is the full transitive freeze (158 packages). Rebuild: `python3.12 -m venv ~/.video-tts-env && source ~/.video-tts-env/bin/activate && pip install -r requirements-video-ai.lock`.
