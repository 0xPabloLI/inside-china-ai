...
- **Already applied:**
  ...
  4. Cross-stage signal reuse: trend discovery's `extractScript` extracts `imageUrl` alongside article titles from the same DOM — one CDP request produces both article metadata and image URL signals. Asset sourcer's Phase 0 `loadCachedImages` consumes these cached URLs from `trending-topics.json` — zero additional CDP requests for images already seen.
  5. Dual-capability CDP sources: all 9 CDP search sources have both `capabilities.articles` and `capabilities.images`. `extractScript` returns `{ url: articleUrl, imageUrl }` for trend discovery; `primaryScript` returns `{ url: imageUrl, type: 'image' }` for asset sourcing. Same search-page DOM, zero redundant requests.
