...
function NewsHub() {
  ...
  const { topic: topicId } = useSearch({ from: "/news" });
  ...
  return (
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-5xl px-6 pt-12 pb-24">
        ...
        {/* Featured topic page */}
        <Link
          to="/news/chinese-ai-models"
          className="group mt-6 flex flex-col justify-between gap-2 rounded-lg border border-border/60 p-5 transition-colors hover:bg-accent/50 sm:flex-row sm:items-center"
        >
          <div>
            <div className="text-xs uppercase tracking-wider text-muted-foreground">
              Topic page · 中国AI模型
            </div>
            <h2 className="mt-1 font-serif text-2xl leading-snug group-hover:underline">
              Chinese AI models
            </h2>
            <p className="mt-1 text-sm text-muted-foreground">
              Every DeepSeek, Qwen, Kimi and GLM release we've covered, plus the keywords and
              benchmarks that matter.
            </p>
          </div>
          <span className="shrink-0 text-sm font-medium">Open topic →</span>
        </Link>
