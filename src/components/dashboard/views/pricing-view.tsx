  ...
  return (
    <div className="space-y-4">
        ...
        <div className="flex gap-1 rounded-full border border-border/60 bg-muted/40 p-0.5">
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              mode === "output" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {isZh ? "输出" : "Output"}
          </button>
          <button
            ...
            className={`rounded-full px-3 py-1 text-xs font-medium transition-colors ${
              mode === "input" ? "bg-primary/10 text-primary" : "text-muted-foreground hover:text-foreground"
            }`}
          >
            {isZh ? "输入" : "Input"}
          </button>
        ...
        <table className="w-full border-collapse text-[11px]">
          <thead>
            <tr className="border-b border-border/60">
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "厂商" : "Vendor"}</th>
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "模型" : "Model"}</th>
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "地区" : "Region"}</th>
              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">{isZh ? `输入 (元/M)` : "Input (USD/M)"}</th>
              <th className="px-2 py-1.5 text-right font-semibold text-muted-foreground">{isZh ? `输出 (元/M)` : "Output (USD/M)"}</th>
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "上下文" : "Context"}</th>
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "档次" : "Tier"}</th>
              <th className="px-2 py-1.5 text-left font-semibold text-muted-foreground">{isZh ? "来源" : "Source"}</th>
            </tr>
          </thead>
          <tbody>
            {allModels.map((m, i) => {
              ...
              return (
                <tr key={i} className="border-b border-border/20 hover:bg-muted/20">
                  <td className="px-2 py-1.5 font-semibold" style={{ color: m.color }}>{vn}</td>
      ...
      <div className="text-[11px] leading-relaxed text-muted-foreground/70">
        <div className="mb-1 font-semibold text-muted-foreground">{isZh ? "数据来源（均已验证）" : "Data Sources (All Verified)"}</div>
        <div className="mb-2">
        ...
        <div className="mb-2">
          ...
          {isZh ? `（官方定价页面，USD→RMB 1:${EXCHANGE_RATE.toFixed(2)}）` : ` (official pricing pages, USD)`}:
          <br />
